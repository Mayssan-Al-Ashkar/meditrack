...
abstract class _IntakeMedicationViewModelBase with Store, BaseViewModel {
  ...
  TextEditingController numberOfReminderController = TextEditingController();
  ...
  Future<void> pickTime() async {
    TimeOfDay? picked =
      await showTimePicker(context: viewContext!, initialTime: TimeOfDay.now());
    if (picked != null) {
  ...
  List<ReminderModel> getListOfReminderModel(InventoryModel medication) {
    ...
    //default 10 alarm
    int n = int.tryParse(numberOfReminderController.text) ?? 10;
    final selTime = _selectedtime ?? TimeOfDay.now();
    final medDate = date.add(Duration(hours: selTime.hour, minutes: selTime.minute));
  ...
  DateTime _getNextDate(DateTime date) {
    ...
    return date; // fallback, should not happen if period is valid
  }
