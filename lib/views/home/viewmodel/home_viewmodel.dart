...
abstract class _HomeViewmodelBase with Store, BaseViewModel {
  ...
  // CalendarController was removed/changed in newer package versions. Keep
  // a nullable controller so migration to the new API can be done later.
  // TableCalendar v3 API: use focusedDay, selectedDay, and calendarFormat
  ...
  void init() async {
    changeLoading();
    denemeGet();
    final _selectedDay = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day);
  ...
  void onDaySelected(DateTime day, DateTime focusedDay) {
    ...
    selectedEvents = this.events[day] ?? [];
  ...
  void onPageChanged(DateTime focusedDay) {
    this.focusedDay = focusedDay;
    print('CALLBACK: _onPageChanged: $focusedDay');
  }

  Future<bool> storeReminders() async {
