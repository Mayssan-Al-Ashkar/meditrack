...
abstract class _PharmacyViewModelBase with Store, BaseViewModel {
  late PharmacyService _pharmacyServiceNew;
  ...
  Future<void> getClosestPharmacy() async {
    ...
    for (int i = 0; i < pharmacies.length; i++) {
      final lat = double.tryParse(pharmacies[i].lat ?? '') ?? 0.0;
      final long = double.tryParse(pharmacies[i].long ?? '') ?? 0.0;
      double distance = calculateDistance(lat, long,
          userPosition.latitude,
          userPosition.longitude);
      pharmacies[i].distance = distance;
    }
    pharmacies.sort((a, b) => ((a.distance ?? 0.0) - (b.distance ?? 0.0)).toInt());
  }
  ...
  GoogleMap displayPharmacyOnMap() {
    try {
      return GoogleMap(
        ...
        initialCameraPosition: pharmacyToCameraPosition(closestPharmacy ?? Pharmacy()),
        onMapCreated: (map) {
          mapsInit(map);
        },
  ...
  Pharmacy? theClosestPharmacyModel() {
    ...
    for (Pharmacy pharmacy in pharmacies) {
      if ((closest.distance ?? 0.0) > (pharmacy.distance ?? 0.0)) {
        closest = pharmacy;
      }
    }
    return closest;
  ...
  CameraPosition pharmacyToCameraPosition(Pharmacy pharmacy) {
    final double lat = double.tryParse(pharmacy.lat ?? '') ?? 0.0;
    final double lon = double.tryParse(pharmacy.long ?? '') ?? 0.0;
  ...
  Set<Marker> _createMarker() {
    return pharmacies
        .map((element) => Marker(
            ...
            position: LatLng(
              double.tryParse(element.lat ?? '') ?? 0.0, double.tryParse(element.long ?? '') ?? 0.0),
            icon:
